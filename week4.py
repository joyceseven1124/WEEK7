...
def response_find_member(): 
    state = session["state"]

 
    if state == "已登入" and request.method == "GET":
        user = request.args.get("username",None)
        user_data = conn.api_member(user)
        if user_data != None : 
            data = {"data":{
                      "id":user_data[0],
                      "name":user_data[1],
            ...
            data = {"data":user_data}
            return jsonify(data)
    elif state == "未登入" and request.method == "GET":
        user_data = conn.api_member("")
        data = {"data":user_data}
        return jsonify(data)
    
    
    elif state == "已登入" and request.method == "PATCH":
        input_newname = json.loads(request.data)
        if input_newname:
            print(input_newname)
            newname = input_newname['name']
            id = session["id"] 
            conn.api_update(newname,id)
            session["name"] = newname
            return  jsonify({"ok":True})
        else:
            return  jsonify({"error":True})
        
    elif state == "未登入" and request.method == "PATCH":
        return  jsonify({"error":True})
